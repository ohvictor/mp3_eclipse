...
void SPI_Init(uint8_t spi_port, SPI_config_t config)
{
    ...
    if(config.mode)
    {
        ...
        SPI_PTRS[spi_port]->CTAR[1] |= SPI_CTAR_FMSZ(config.frame_size)-1; // User defined frame size
        ...
        //Clock scaler = 1024 => CLK = (SYSCLK/5) * (1/1024) = 9765.625 (Parecido a UART)
...
void SPI_Write(uint8_t spi_port, uint16_t spi_data, uint8_t chip_select_bit_vector)
{
	SPI_PTRS[spi_port]->PUSHR = (SPI_PTRS[spi_port]->PUSHR & ~SPI_PUSHR_PCS_MASK) | SPI_PUSHR_PCS(1 << chip_select_bit_vector) |
								(SPI_PTRS[spi_port]->PUSHR & ~SPI_PUSHR_TXDATA_MASK) | SPI_PUSHR_TXDATA(spi_data);
}
