...
#define MATRIX0	(uint16_t) 25
#define MATRIX1 (uint16_t) 50
...
static void LEDMatrix_RGB2PWM()
{
    ...
    uint8_t brightness_ratio = (uint8_t)((matrix_brightness * 255) / MAX_BRIGHTNESS);


	for(uint8_t x = 0; x < HEIGHT; x++){
		for(uint8_t y = 0; y < WIDTH; y++){

			uint8_t red_brightness = (uint8_t)((uint16_t)rgb_led_matrix[x][y].r * brightness_ratio >> 8);
			uint8_t green_brightness = (uint8_t)((uint16_t)rgb_led_matrix[x][y].g * brightness_ratio >> 8) ;
			uint8_t blue_brightness = (uint8_t)((uint16_t)rgb_led_matrix[x][y].b * brightness_ratio >> 8) ;
