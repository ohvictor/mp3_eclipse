...
__ISR__ DMA5_IRQHandler()
{
	/* Clear the interrupt flag. */
	DMAptr->CINT |= DMA_CINT_CINT(5); // Creo que es asi
	DMA_ISRhandler(5);
}

__ISR__ DMA6_IRQHandler()
{
	/* Clear the interrupt flag. */
	DMAptr->CINT |= DMA_CINT_CINT(6); // Creo que es asi
	DMA_ISRhandler(6);
}

__ISR__ DMA7_IRQHandler()
{
	/* Clear the interrupt flag. */
	DMAptr->CINT |= DMA_CINT_CINT(7); // Creo que es asi
	DMA_ISRhandler(7);
}

__ISR__ DMA8_IRQHandler()
{
	/* Clear the interrupt flag. */
	DMAptr->CINT |= DMA_CINT_CINT(8); // Creo que es asi
	DMA_ISRhandler(8);
}

__ISR__ DMA9_IRQHandler()
{
	/* Clear the interrupt flag. */
	DMAptr->CINT |= DMA_CINT_CINT(9); // Creo que es asi
	DMA_ISRhandler(9);
}

__ISR__ DMA10_IRQHandler()
{
	/* Clear the interrupt flag. */
	DMAptr->CINT |= DMA_CINT_CINT(10); // Creo que es asi
	DMA_ISRhandler(10);
}

__ISR__ DMA11_IRQHandler()
{
	/* Clear the interrupt flag. */
	DMAptr->CINT |= DMA_CINT_CINT(11); // Creo que es asi
	DMA_ISRhandler(11);
}

__ISR__ DMA12_IRQHandler()
{
	/* Clear the interrupt flag. */
	DMAptr->CINT |= DMA_CINT_CINT(12); // Creo que es asi
	DMA_ISRhandler(12);
}

__ISR__ DMA13_IRQHandler()
{
	/* Clear the interrupt flag. */
	DMAptr->CINT |= DMA_CINT_CINT(13); // Creo que es asi
	DMA_ISRhandler(13);
}

__ISR__ DMA14_IRQHandler()
{
	/* Clear the interrupt flag. */
	DMAptr->CINT |= DMA_CINT_CINT(14); // Creo que es asi
	DMA_ISRhandler(14);
}

__ISR__ DMA15_IRQHandler()
{
	/* Clear the interrupt flag. */
	DMAptr->CINT |= DMA_CINT_CINT(15); // Creo que es asi
	DMA_ISRhandler(15);
}


__ISR__ DMA_Error_IRQHandler()
{
	/* Clear the error interrupt flag.*/
	DMAptr->CERR |= 0;

	//PORTB->PTOR |= (1 << PIN_RED_LED); //Sacar
